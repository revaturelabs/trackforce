...
public class AssociateDaoImpl implements AssociateDao {
	...
	/**
	 * Gets list of associates matching criteria. Used by updated angular front end
	 * to perform Pagination of results and improve performance.
	 * 
	...
	public List<TfAssociate> getNAssociateMatchingCriteria(int startIdx, int numRes, int mktStatus, int clientId,
			String sortText, String firstName, String lastName) {
		...
		if (!firstName.isEmpty() && !lastName.isEmpty()) {
			criteria.where(builder.and(builder.like(expFirst, firstName.toLowerCase()),
					builder.like(expLast, lastName.toLowerCase())));
		} else if (!sortText.isEmpty()) {
			if (clientId == -1 && mktStatus != -1) {
				criteria.where(
						builder.and((builder.or(builder.like(expFirst, sortText), builder.like(expLast, sortText))),
				...
				criteria.where(
						builder.and((builder.or(builder.like(expFirst, sortText), builder.like(expLast, sortText))),
								builder.and(builder.equal(root.get(MKTSTS), mktStatus),
										builder.equal(root.get(CLIENT), clientId))));
			} else {
				criteria.where(builder.or(builder.like(expFirst, sortText), builder.like(expLast, sortText)));
			}
		} else {
			if (clientId == -1 && mktStatus != -1) {
				...
				criteria.where(builder.and(builder.equal(root.get(MKTSTS), mktStatus),
						builder.equal(root.get(CLIENT), clientId)));
			}
		}
		...
		List<TfAssociate> results = session.createQuery(criteria).setCacheable(true).setFirstResult(startIdx)
				.setMaxResults(numRes).getResultList();
	...
	public TfAssociate getAssociate(Integer id) {
		...
		return HibernateUtil.runHibernate((Session session, Object... args) -> session
				.createQuery("from TfAssociate a where a.id = :id", TfAssociate.class).setParameter("id", id)
	...
	public List<TfAssociate> getNAssociates() {
		...
		return HibernateUtil.runHibernate(
				(Session session, Object... args) -> session.createQuery("from TfAssociate", TfAssociate.class)
						.setMaxResults(60).setCacheable(true).getResultList());
	}

	/**
	 * @author Art B. getAllStatusCounts returns a HashMap of Integer keys and
	 *         values, representing the number of associates (value) for each status
	 *         code id (key). Below is what each id represents: 1. Mapped: Training
	 *         (name is null in the database) 2. Mapped: Reserved 3. Mapped:
	 *         Selected 4. Mapped: Confirmed 5. Mapped: Deployed 6. Unmapped:
	 *         Training 7. Unmapped: Open 8. Unmapped: Selected 9. Unmapped:
	 *         Confirmed 10. Unmapped: Deployed 11. Directly Placed 12. Terminated
	 */
	...
	/**
	 * @author Art B. Sorts the values retrieved from getAllStatusCounts into the
	 *         order that the AssociateResource uses them, and converts it to a
	 *         HashMap with a String keys instead that represent the names. Removes
	 *         'Directly Placed' and 'Terminated' values, and adds "Undeployed
	 *         Mapped" and "Undeployed Unmapped" aggregate values in the process.
	 * 
	 */
	...
	//v1811
	//This is a previous iteration update method for TfAssociate.
	//Presumbly just for updating the first and last name of an associate at their homepage.
	//Doesn't seem to be called by anything.
	@Override
	...
	/**
	 * Gets undeployed (mapped or unmapped). Optimized to remove redundancy @author
	 * Paul C.-1807
	 */
	@Override
...
/*	Nested if statements to prevent shortcircuit if statements from throwing a nullpointerexception on the hibernate transactions
	Seems to throw the error if one of the params are set to null and they try to compare it from the database.
	Current if statements work appropriately to ensure that data is non-null and valid to update for the database.
	Current Variables not updated are: TfBatch - batch. TfClient - endclient. Set<TfInterview> - interview.
										Set<TfPlacement> - placement. Timestamp - clientStartDate
	These variables are not currently being sent as data to the Java-side, so v1811 implementation has not added them to be updated
	accordingly.*/
	@Override
	public boolean updateAssociate(TfAssociate associate) {
		...
		return runHibernateTransaction((Session session, Object... args) -> {
			TfAssociate temp = new TfAssociate();
			temp = session.get(TfAssociate.class, associate.getId());
			if (associate.getFirstName() != null) {
				if (!associate.getFirstName().equals("")) {
					temp.setFirstName(associate.getFirstName());
				}
			}
			if (associate.getLastName() != null) {
				if (!associate.getLastName().equals("")) {
					temp.setLastName(associate.getLastName());
				}
			}
			if (associate.getStagingFeedback() != null) {
				if (!associate.getStagingFeedback().equals("")) {
					temp.setStagingFeedback(associate.getStagingFeedback());
				}
			}
			if (associate.getClient() != null) {
	 ...
	 * 
	 * @Override public <T> T countMappedAssociatesByValue(String column, T value,
	 * Integer mappedStatus) { Sessional<T> ss = (Session session, Object... args)
	 * -> { String condition = null;
	 * 
	 * if (Integer.valueOf(value.toString()) != -1) {
	 * 
	 * condition = column + " = " + args[0] + " AND "; } else { condition = ""; }
	 * String hql = "SELECT COUNT(TF_ASSOCIATE_ID) FROM TfAssociate WHERE " +
	 * condition + "TF_MARKETING_STATUS_ID = :status"; Query query =
	 * session.createQuery(hql).setCacheable(true); return (T)
	 * query.setParameter("status", args[1]).getSingleResult(); }; return
	 * HibernateUtil.runHibernate(ss, value, mappedStatus); }
	 */

	@Override
	/*
	 * Sends back number of Associates compared against Column name. [Passed as a
	 * string-variable]. Value - Conditional Clause against the Column name.
	 * TF_Client_Id - "some id variable" mappedStatus - the marketing status of
	 * mapped Associates.
	 */
	public long countMappedAssociatesByValue(String column, String value, int mappedStatus) {
		LogUtil.logger
				.trace("Number of Mapped Associates based of what ColumnName: " + column + " and the value: " + value);
		Sessional<Long> ss = (Session session, Object... args) -> {
			...
			String hql = "SELECT COUNT(TF_ASSOCIATE_ID) FROM TfAssociate WHERE " + column
					+ " = :value AND TF_MARKETING_STATUS_ID = :status";
	...
	// Should probably only be used for testing. Deleting records for production is
	// a bad idea.
	@Override
	...
	/*
	 * The method deleteOldAssociateProcedure() will call a stored procedure on the
	 * database side. the procedure is meant to delete associates who are pending
	 * approval for over 30 days. Only deletes associates, not trainers, etc.
	 */
	@Override
	public void deleteOldAssociateProcedure() {
		LogUtil.logger.trace("Automatic Deleting Unapproved Old Associates.");
		runHibernateTransaction((Session session, Object... args) -> {
			StoredProcedureQuery query = session.createStoredProcedureCall("delete_old_associates")
