...
public class BatchResource {
	 ...
	 *         <p>
	 *         Gets all batches, optionally filtered by start and end date query
	 *         parameters For example, sending a GET request to
	...
	public Response getAllBatches(@QueryParam("start") Long startDate, @QueryParam("end") Long endDate,
		...
		List<TfBatch> batches = getBatchService().getAllBatches();
		...
		if (authorizedRoles.contains(role)) {
			if (startDate != null && endDate != null) {
				...
				// iterates through the list of batches and removes those that are before the
				// start and after the end date
	...
	public Response getBatchAssociates(@PathParam("id") Integer id, @HeaderParam("Authorization") String token) {
		...
		Set<TfAssociate> associates = getBatchService().getBatchById(id).getAssociates();
	...
	public Response getBatchDetails(@QueryParam("start") Long startDate, @QueryParam("end") Long endDate,
		...
		if (authorizedRoles.contains(role)) {
			...
			logger.info("User has insufficient Privilieges. Forbidden Access");
			status = Status.FORBIDDEN;
			return Response.status(status).entity(JWTService.invalidTokenBody(token)).build();
		}

		JSONObject batchDetails = getJsonObj();
		JSONArray batchesJ = getJsonArray();

		BatchDaoImpl bd = getBatchDaoImpl();
		List<TfBatch> batches = bd.getBatchesForPredictions(courseName, new Timestamp(startDate),
				new Timestamp(endDate));

		for (TfBatch batch : batches) {
			int unmappedCount = getUnmappedCount(batch.getAssociates());

			JSONObject b = getJsonObj();
	...
	/**
	 * 1806_Austin_M Iterate through set of associates and increment count based on
	 * associate status.
	 * 
	 * @param associates
	...
	/**
	 * 1806_Chris_P Super similar to the previous method, except that this one only
	 * returns the aggregate count of all associates in a particular curriculum
	 * selected from the Predictions Page.
	 */
	@GET
	@ApiOperation(value = "Returns associates for batch", notes = "Returns list of associates for a specific batch based on batch id.")
	@Path("/countby")
	...
	// 1806_Andrew_H gets all batches within a certain date range, used in
	// batch-details
	@GET
	...
	public Response getBatchInfo(@PathParam("id") Integer id, @HeaderParam("Authorization") String token) {
		...
		BatchDaoImpl bd = getBatchDaoImpl();
		TfBatch batch = getBatchService().getBatchById(id);

		return Response.status(status).entity(batch).build();
	}

	// For use in mocking tests
	...
	public BatchDaoImpl getBatchDaoImpl() {
