 ...
 * 
 * Checks to make sure user has a valid token and is an admin
 *
 */
@Provider //denotes a filter
@Admin //binds it to an interface, so it can be used as an annotation
public class AdminFilter implements ContainerRequestFilter {
	 ...
	 * 
	 * @return nothing if the user is an admin, otherwise an WebApplicationException
	 * is thrown
	 * 
	 */
	@Override
	public void filter(ContainerRequestContext request) throws IOException {
		...
		boolean status = jwt.isAdmin(token);
