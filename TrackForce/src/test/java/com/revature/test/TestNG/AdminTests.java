...
public class AdminTests {
	...
	/*
	 * stores the values of dates in the batch list page, first element is the start
	 * date of first batch & second element is the end date of the first batch and
	 * so on... because start date and end dates dont have unique identifiers
	 * 
	 */
	...
	// Method from stackOverFlow to generate a random string (no invalid characters)
	// -- used to generate a valid username
	...
	// Method from stackOverFlow to generate a random string (with invalid
	// characters) -- used to generate an invalid username
	...
	void adminCanCreateValidAssociate() {
		...
		wait.until(ExpectedConditions
				.presenceOfElementLocated(By.cssSelector("div[class='alert alert-success ng-star-inserted']")));
	...
	void adminCannotCreateInvalidAssociate() {
		...
		String message = wd.findElement(By.xpath("/html/body/app-component/div/app-create-user/form/fieldset[1]/div"))
				.getText();
	...
	/*
	 * There's a non-repeatable bug where sometimes the clientList is loaded twice.
	 * This test tests if the values in the client list are unique.
	 * 
	 */
	...
	/*
	 * There's a defect where the filter functionality on the batch list tab for the
	 * admin user does not work at edge cases. For example if a batch starts
	 * 12/1/2017 and ends 11/2/2018 and you try to filter by inputing start date as
	 * 12/1/2018 and end date 11/2/2018 it wont show any results, you have to input
	 * one day before start date and one day after end date for it to work.
	 * 
	 * This test grabs the start date and end date of the first batch and checks if
	 * it shows up after filtering.
	 */
	...
	/*
	 * The name of this method should be changed if we can not fixed it. There are
	 * so potential errors to this method. Partly because we do not fully understand
	 * how to grab web elements from a pop window and select from a dropdown on the
	 * pop up
	 * 
	...
	/*
	 * Adding test according the notes on the issues tap
	 * abc
	 */
	@Test(priority = 21)
	public void predictionTab() {
		int jtaNeeded = new Random().nextInt(50);
		
		wd.findElement(By.id("predictions")).click();
		wd.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		wait.until(ExpectedConditions.elementToBeClickable(wd.findElement(By.id("JTA-input")))).click();			
		wd.findElement(By.id("JTA-input")).sendKeys(String.valueOf(jtaNeeded));
		wait.until(ExpectedConditions.elementToBeClickable(wd.findElement(By.id("check-resources")))).click();
		
		wd.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		wait.until(ExpectedConditions.elementToBeClickable(wd.findElement(By.id("needed")))).click();
		
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("batchDetailsHeader")));
	}
