...
public class DbUnit extends JdbcBasedDBTestCase {
	...
	public DbUnit(String name) {
		...
		System.setProperty(PropertiesBasedJdbcDatabaseTester.DBUNIT_CONNECTION_URL,
				"jdbc:oracle:thin:@trackforce.chgtukwhhsrg.us-east-1.rds.amazonaws.com:1521:ORCL");
	...
	protected JdbcDatabaseTester getDatabaseTester() throws Exception {
		JdbcDatabaseTester tester = new JdbcDatabaseTester(this.getDriverClass(), this.getConnectionUrl(),
	...
	// modify default DbUnit clean up and set up operations to clean up database
	// after test
	/*
	 * http://dbunit.sourceforge.net/components.html#cleanInsert This operation
	 * literally refreshes dataset contents into the target database. This means
	 * that data of existing rows are updated and non-existing row get inserted. Any
	 * rows which exist in the database but not in dataset stay unaffected. This
	 * approach is more appropriate for tests that assume other data may exist in
	 * the database.if they are correctly written, tests using this strategy can
	 * even be performed on a populated database like a copy of a production
	 * database.
	 */
