...
public class ClientResourceTest {
	...
	/**
	 * Unhappy path testing for testGetAllClients, tests thatn a 401 status code is
	 * given if a request is made with a bad token
	 * 
	 * @author Katelyn B Written Nov. 4, 2018, Batch 1809
	 */
	...
	/**
	 * Unhappy path testing for testGetAllClient, tests that a 404 status code is
	 * generated for a bad URL
	 * 
	 * @author Katelyn B Written Nov. 4, 2018, Batch 1809
	 */
	@Test(priority = 10)
	...
	/**
	 * Unhappy path testing for testGetAllClient, tests that a 405 status code is
	 * given for a bad html verb
	 * 
	 * @author Katelyn B Written Nov. 4, 2018, Batch 1809
	 */
	@Test(priority = 10)
	public void testGetAllClientsBadVerb() {
		Response response = given().header("Authorization", token).when().post(URL + "/getAll/").then().extract()
				.response();
	...
	public void testUserRole() {
		...
		// assocToken).when().post(URL + "getAll").then().extract().response();
		// assertTrue(response.statusCode() == 403);
	}

	@DataProvider(name = "urls")
	public String[] getURLs() {
		return new String[] { "/getAll/", "/associates/get/0", "/mapped/get/", "/50/" };
	}
	 ...
	 * verb. 2. uses a service without a token 3. uses a bad token. 4. the role in
	 * the token deemed the user unauthorized to use the service As this method of
	 ...
	 * @author Seth L.
	 * @param method
	 *            - comma-separated list of HTTP Verbs that the service uses
	 * @param url
	 *            - the uri for the service
	 * @param needAuth
	 *            - if the user has to be a higher level of authorization than
